...
import {getList} from "../../api/ProjectIssueApi";
...
const ListComponent = () => {
    ...
    const handleClickAdd = () => {
        ...
        if (
            token?.roleNames?.includes('INTERN') ||
            token?.roleNames?.includes('CONTRACT_WORKER') ||
            token?.roleNames?.includes('STAFF')
        ) {
            alert("권한이 없습니다.");
            return;
        }
    ...
    const handleClickMyList = () => {
        if (token?.mno) {
            navigate(`/project/list/${token?.mno}`)
        } else {
            alert("로그인 후 이용 가능합니다.")
        }
    }
    ...
    return (
        <div>
            ...
            <Card>
                <CardBody>
                    <CardTitle tag="h5" className="d-flex justify-content-between align-items-center mb-3">프로젝트 리스트
                        <Button color="secondary" outline className="me-2" onClick={handleClickMyList}>내 리스트 보기</Button>
                    ...
                    <Table className="no-wrap mt-3 align-middle" borderless>
                        ...
                        {serverData.dtoList.map((project, index) => (
                            <tr key={project.pno} className="border-top"
                                onClick={() => moveToRead(project.pno, token?.mno)}>
                                <td>
