...
import {getIssueNullList, getMyList} from "../../api/ProjectIssueApi";
...
const ProjectIssueMyListComponent = () => {
    ...
    // 이동에 관련된 변수들
    const {page, size, moveToProjectIssueNullList, moveToMyIssueRead} = useProjectMove()
    ...
    const handleClickSort = (sort) => {
        ...
        navigate(`/project/issue/nulllist/${pno}?${queryStr}`); // 새로운 쿼리로 url이동
    };
    ...
    useEffect(() => {
        ...
        setFetching(true)
        getIssueNullList(pno, {page: currentPage, size: currentSize, sort: currentSort, order: order}).then(data => {
            setServerData(data);
            setFetching(false)
        });
    }, [location.search, pno]);
    ...
    return (
        <div>
            ...
            <PageComponent serverData={serverData}
                           movePage={(pageParam) => moveToProjectIssueNullList(pno, pageParam)}></PageComponent>
        </div>
    )
}
