...
import {getWriterList} from "../../api/DocumentApi";
...
const DocumentRequestListComponent = () => {
    ...
    // 이동에 관련된 값들
    const {searchText, searchType, page, size, moveToList, moveToRead, moveToAdd} = useDocumentMove()
    ...
    console.log("Rendered writerCount:", writerCount);  // 렌더링된 후 출력
    // 정렬 버튼 클릭
    const handleClickSort = (sort) => {
        ...
        navigate(`/document/writer/list/${writer}?${queryStr}`); // Navigate with the new query string
        setFetching(false)
    };
    ...
    const handleSearch = () => {
        ...
        navigate(`/document/writer/list/${writer}?${queryStr}`);
    };
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                // List 메서드 호출
                const data = await getWriterList(writer, {
                    page: currentPage,
                    size: currentSize,
                    sort: currentSort,
                    order: order,
    ...
    return (
        <div>
            ...
            <Card>
                <CardBody>
                    <CardTitle tag="h5" className="d-flex justify-content-between align-items-center">내가 결재 요청한 문서
                    ...
                    <Row className="mb-3">
                        ...
                        <Col sm="3" className="d-flex align-items-end">
                            ...
                            <Button color="warning" onClick={() => navigate(`../document/writer/list/${writer}`)}>검색조건
                                초기화</Button>
                        </Col>
                    </Row>
