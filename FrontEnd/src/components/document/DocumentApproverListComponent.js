...
import {getApproverList} from "../../api/DocumentApi";
...
const DocumentRequestListComponent = () => {
    ...
    // 이동에 관련된 값들
    const {searchText, searchType, page, size, moveToApproverList, moveToRead, moveToAdd} = useDocumentMove()
    ...
    const handleClickSort = (sort) => {
        ...
        navigate(`/document/approver/list/${token?.mno}?${queryStr}`); // Navigate with the new query string
        setFetching(false)
    };
    ...
    const handleSearch = () => {
        ...
        navigate(`/document/approver/list/${mno}?${queryStr}`);
    };
    ...
    useEffect(() => {
        ...
        const currentSort = queryParams.get("sort") || "dno"; // 기본값 dno
        ...
        const fetchData = async () => {
            setFetching(true); // 데이터 요청 시작할 때 로딩 시작
            try {
                // List 메서드 호출
                const data = await getApproverList(mno, {
                    page: currentPage,
                    size: currentSize,
                    sort: currentSort,
                    order: order,
                ...
                setFetching(false); // 모든 데이터 로드 후 로딩 종료
            }
        };
    ...
    return (
        <div>
            ...
            <Card>
                <CardBody>
                    <CardTitle tag="h5" className="d-flex justify-content-between align-items-center">내가 결재해야할 문서
                    ...
                    <Row className="mb-3">
                        ...
                        <Col sm="3" className="d-flex align-items-end">
                            ...
                            <Button color="warning" onClick={() => navigate(`../document/approver/list/${mno}`)}>검색조건
                                초기화</Button>
                        </Col>
                    </Row>
                        ...
                        {serverData.dtoList.map((document, index) => (
                            <tr key={document.dno} className="border-top"
                                ...
                                <td>
                                    <div className="ms-3">
                                        <h6 className="mb-0">
                                            {document.approved === "REQUESTED" ? "결재 요청"
                                                : document.approved === "APPROVED" ? "결재 완료"
                                                    : document.approved === "REJECTED" ? "반려" : ""}
                                        </h6>
                                    </div>
                                </td>
                            </tr>
            ...
            <PageComponent serverData={serverData} movePage={moveToApproverList}></PageComponent>
        </div>
    )
}
export default DocumentRequestListComponent;