...
import {getList} from "../../api/DocumentApi";
...
const DocumentRequestListComponent = () => {
    ...
    // 이동에 관련된 값들
    const {searchText, searchType, page, size, moveToRequestedList, moveToRead, moveToAdd} = useDocumentMove()
    ...
    const handleSearch = () => {
        ...
        navigate(`/document/requested/list?${queryStr}`);
    };
    ...
    const handleClickSort = (sort) => {
        ...
        navigate(`/document/requested/list?${queryStr}`); // Navigate with the new query string
        setFetching(false)
    };
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                setServerData(data);
                console.log(data);
                // 모든 멤버의 mno에 대해 statusRead 호출을 병렬로 처리
            } catch (error) {
    ...
    return (
        <div>
            ...
            <Card>
                <CardBody>
                    <CardTitle tag="h5" className="d-flex justify-content-between align-items-center">결재 대기중인 문서
                    ...
                    <Row className="mb-3">
                        ...
                        <Col sm="3" className="d-flex align-items-end">
                            ...
                            <Button color="warning" onClick={() => navigate(`../document/requested/list`)}>
                                검색조건 초기화
                            </Button>
                        </Col>
                    </Row>
            ...
            <PageComponent serverData={serverData} movePage={moveToRequestedList}></PageComponent>
        </div>
    )
}
