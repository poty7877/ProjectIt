...
import {getRejectedList} from "../../api/DocumentApi";
...
const DocumentRejectedListComponent = () => {
    ...
    // 이동에 관련된 값들
    const {searchText, searchType,page, size, moveToRejectedList, moveToRead, moveToAdd} = useDocumentMove()
    ...
    const handleClickSort = (sort) => {
        ...
        navigate(`/document/rejected/list?${queryStr}`); // Navigate with the new query string
    };
    ...
    const handleSearch = () => {
        ...
        navigate(`/document/rejected/list?${queryStr}`);
    };
    ...
    useEffect(() => {
        ...
        const currentSort = queryParams.get("sort") || "dno";
        const currentOrder = queryParams.get("order") || "";
        let order = currentOrder === "asc";
        ...
        const fetchData = async () => {
            setFetching(true);
            try {
                const data = await getRejectedList({
                    page: currentPage,
                    size: currentSize,
                    sort: currentSort,
                    order: order,
    ...
    return (
        <div>
            ...
            <Card>
                <CardBody>
                    <CardTitle tag="h5" className="d-flex justify-content-between align-items-center">결재 반려된 문서
                    ...
                    <Row className="mb-3">
                        ...
                        <Col sm="3" className="d-flex align-items-end">
                            ...
                            <Button color="warning" onClick={() => navigate(`../document/rejected/list`)}>검색조건 초기화</Button>
                        </Col>
                    </Row>
            ...
            <PageComponent serverData={serverData} movePage={moveToRejectedList}></PageComponent>
        </div>
    )
}
export default DocumentRejectedListComponent;